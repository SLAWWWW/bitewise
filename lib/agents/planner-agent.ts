...
SAFETY MARGIN — do not skip this: the ${input.hoursUntilExpiry.toFixed(1)}-hour figure is the moment this food becomes unsafe to eat, not a deadline you can plan up to. Food that arrives at, or after, that moment cannot actually be distributed or eaten — the beneficiary needs real time on the other end to hand it out and for people to eat it. The final delivery stage's end time must land clearly before the full window elapses, never at or after it. As a rule of thumb, finish delivery with at least 15% of the window (or 10 minutes, whichever is greater) still remaining — if your plan can't fit that margin given the real fleet/storage constraints, say so explicitly in that stage's risk_note rather than silently planning up to the wire.

${
  input.directPartnerAllocation
    ? `Produce a stage-by-stage plan from pickup through to delivery. Use exactly these stage kinds, in order: pickup, transport, storage, delivery — no listing stage, no contingency stage, since this item is already committed to a partner and never publicly listed. Ground every timing in the ${input.hoursUntilExpiry.toFixed(1)}-hour spoilage window, respecting the safety margin above. For the required contingency object (a separate field, not a stage), the beneficiary must be ${input.directPartnerAllocation.beneficiaryName} again — describe what happens if this specific delivery run is disrupted (vehicle unavailable, site closed), not an alternate claimant, since there is no public claiming for this item. Keep every field to one or two sentences.`
    : `Produce a stage-by-stage plan from pickup through to the food being eaten. Use these stage kinds in order: pickup, transport, storage, listing, contingency, delivery. Ground every timing in the ${input.hoursUntilExpiry.toFixed(1)}-hour spoilage window and the ${ESCALATION_THRESHOLD_HOURS}-hour escalation rule, respecting the safety margin above. Choose the contingency beneficiary that can actually handle this specific item, and say why in one sentence. Keep every field to one or two sentences.`
}
