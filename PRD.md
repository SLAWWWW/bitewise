...
1. **No authentication or authorization anywhere.** `/approvals` — the NGO staff approval gate — is reachable by anyone who knows the URL. The "requires NGO approval" workflow is a **process/UX boundary, not a security boundary**. Acceptable for a demo/hackathon context; would need real auth (staff login, role separation from the public surface) before any real deployment. **This is now the single largest gap.**
2. **AI provider dependency + cost ceiling.** The agent pipeline depends on Gemini's free-tier quota (15 req/min per model); Section 8.8 documents the mitigations shipped (shortlisting, approval-time reuse, idempotent tools), but sustained high-volume production use would need a paid tier or further batching.
3. **One migration is currently un-applied on the live project** (001–008 confirmed applied via `npm run check:migrations` on 2026-08-12; run it again for current status — this list has gone stale before and will again).
   - `009_recipient_profiles.sql` — without it, `POST /api/profiles` fails outright and the client falls back to a locally-generated identity so claiming is never blocked (§7.8); `POST /api/claims` falls back to an insert without `profile_id`/`pickup_deadline_at`, so the one-active-claim rule and the pickup countdown are both silently inactive — claims still work, exactly like pre-v2.3, just fully anonymous again and with no auto-release.
