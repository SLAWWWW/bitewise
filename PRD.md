...
1. **No real authentication or authorization anywhere.** There's no login system, staff and public views are just different pages, not different privilege levels. A shared staff-key check (`lib/staff-auth.ts`, `STAFF_API_KEY`) now gates the four mutating staff-only routes (approve, reject, pickup confirmation, fleet advance) so a random visitor can't call them from the browser console — but this is a **deterrent, not real authorization**: one shared secret, no per-user identity, no roles. Acceptable for a demo/hackathon context; would need real auth (staff login, role separation) before any real deployment. **This is still the single largest architectural gap**, just no longer a live-demo-embarrassment risk.
